...
public class MetodosCursos {
    ...
    ArchivosCursos archivo;
    
    public MetodosCursos(){
        arrayCursos = new ArrayList<Cursos>();
        
        archivo = new ArchivosCursos();
        arrayCursos = archivo.leerInformacionCompleta();
    }
    
    public void escribirEnArchivo(){
        archivo.crearArchivo();
        System.out.println("Entra en el metodo que escribe en el archivo");
        for (int contador = 0; contador < arrayCursos.size(); contador++){
            archivo.escribirInformacionEnELArhivo(arrayCursos.get(contador));
        }
    }
